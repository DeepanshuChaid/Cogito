...
export const getNotifications = asyncHandler(async (req, res) => {
  ...
  for (const n of rawNotifications) {
    ...
    if (n.type === "BLOG_LIKE" && n.blogId) key = `LIKE_${n.blogId}`;
    if (n.type === "COMMENT" && n.commentId) key = `COMMENT_${n.commentId}`;
    if (n.type === "FOLLOW") key = `FOLLOW_${n.issuerId}`;
  ...
  const aggregatedNotifications = Array.from(map.values()).sort(
    (a, b) => b.createdAt.getTime() - a.createdAt.getTime()
  );

  await redisClient.set(
    cacheKey,
    JSON.stringify(aggregatedNotifications),
    { EX: 300 } // cache 5 min
  );
